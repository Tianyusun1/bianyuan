...
def evaluate_single_model(pred_dir, gt_dir, num_thresh=99, tolerance=2):
    ...
    for pred_file in tqdm(pred_files, desc=f"  评估模型: {os.path.basename(pred_dir)}", leave=False):
        ...
        # [修改点]: 使用支持中文路径的读取方式替代 cv2.imread
...
def evaluate_scenario(base_dir, gt_dir, num_thresh=99, tolerance=2):
    ...
    for model_name, metrics in sorted_models:
        print(
            f"{model_name:<20} | {metrics['ods_f1']:.4f}   | {metrics['ois_f1']:.4f}   | {metrics['ods_p']:.4f}    | {metrics['ods_r']:.4f}   | {metrics['mae']:.4f}")
    print("-" * 80)
...
if __name__ == "__main__":
    # ===============================
    # [修改点]: 替换为您本地 Windows 的绝对路径 (使用 r"" 防止转义)
    # ===============================
    ROOT_DIR = r"C:\Users\酸雨\Desktop\毕设—边缘检测\sketchKeras_pytorch"
    GT_DIR = os.path.join(ROOT_DIR, r"data\test\masks")
